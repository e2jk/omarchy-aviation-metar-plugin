...
| `scripts/run-qmllint.sh` | QML syntax/lint on `BarWidget.qml`/`Panel.qml` (Quickshell's `qs.*` namespace and `Process`'s signal typing aren't resolvable without its full build, so those specific categories are suppressed — detected via `qmllint --help` rather than a hardcoded flag list, since which categories exist varies by Qt version: CI's Ubuntu-packaged qmllint is Qt 6.4.2, well behind a typical dev machine's) | Skips with a note if not found (checks both `qmllint` on `PATH` and Qt6's usual `/usr/lib/qt6/bin/qmllint`) |
